...
A caller authenticates, then sends a `quoteId` + `channel`, and the service
returns the booked **policy** (`policyNo`, status, timestamp, etc.).
...
Send `quoteId` and `channel`; receive the booked policy.

```bash
curl -X POST http://127.0.0.1:8000/policies/book \
  -H "Authorization: Bearer <JWT>" \
  -H "Content-Type: application/json" \
  -d '{"quoteId":"Q-2026-000123","channel":"BANCASSURANCE"}'
```
...
```json
{
  "policyNo": "TTB-BANC-20260613-000001",
...
Valid channels: `BRANCH`, `AGENT`, `ONLINE`, `TELESALES`, `BANCASSURANCE`.

### Behaviour
- **Idempotent per `quoteId`** — re-booking the same quote returns the same policy.
- Booking the same quote on a **different channel** → `409 Conflict`.
- Unknown channel → `422 Unprocessable Entity`.
